...
function dateToTimestamp(date) {
  const staticDate = new Date(Date.UTC(1970, 0, 1));
  const newDate = new Date(date);
  return newDate - staticDate;
}
...
function getNextFriday(date) {
  const time = new Date(date);
  time.setDate(time.getDate() + ((5 - time.getDay() + 7) % 7 || 7));
  return time;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const difTime = Math.abs(start.getTime() - end.getTime());
  return Math.ceil(difTime / 1000 / 60 / 60 / 24 + 1);
}
...
function formatDate(date) {
  const newDate = new Date(date);
  const set = newDate.getUTCHours() < 12 ? 'AM' : 'PM';
  return `${newDate.getMonth() + 1}/${newDate.getUTCDate()}/${newDate.getFullYear()}, ${newDate.getUTCHours() % 12 || 12}:${String(newDate.getMinutes()).padStart(2, '0')}:${String(newDate.getSeconds()).padStart(2, '0')} ${set}`;
}
...
function getWeekNumberByDate(date) {
  ...
  newDate.setDate(newDate.getDate() - dayOfWeek + 3);
  const firstThursday = new Date(newDate.getFullYear(), 0, 4);
  const firstThursdayDay = (firstThursday.getDay() + 6) % 7;
  firstThursday.setDate(firstThursday.getDate() - firstThursdayDay + 3);
  const weekNumber =
    Math.round((newDate - firstThursday) / (7 * 24 * 60 * 60 * 1000)) + 1;
...
function getNextFridayThe13th(date) {
  ...
  while (!find) {
    ...
    const month = newDate.getMonth() + 1 + i;
    const findDate = new Date(year, month - 1, 13);
    if (findDate.getDay() === 5) {
      find = true;
      return findDate;
    }
    i += 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      schedule.push(formatOurDate(currentDate));
